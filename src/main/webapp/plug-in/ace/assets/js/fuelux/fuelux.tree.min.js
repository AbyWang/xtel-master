(function(a, c) {
	var b = function(e, d) {
		...
		this.$element.on("contextmenu", ".tree-folder-header", a.proxy(function(f) {
			d = a(f.currentTarget);
			rightClick(f.currentTarget, d.data());
		}, this));
		//右键事件
		this.$element.on("contextmenu", ".tree-item", a.proxy(function(f) {
			d = a(f.currentTarget);
			//this.$element.trigger("rightClick", d.data())
			rightClick(f.currentTarget, d.data());
		}, this));
		this.render()
	};
	b.prototype = {
								...
								a
										.each(
												...
												function(h, j) {
													...
													if (j.type === "folder") {
														...
														i
																.find(
																		".tree-loader")
																.html(
																		e.options.loadingHTML);
														var k = i
																.find(".tree-folder-header");
														k.data(j);
														if ("icon-class" in j) {
															k
																	.find(
														...
														if (j.type === "item") {
															...
															i
																	...
																	.html(
																			j.name);
															i.data(j);
															if ("additionalParameters" in j
																	&& "item-selected" in j.additionalParameters
													...
													if (f
															.hasClass("tree-folder-header")) {
														f
																.parent()
																.find(
																		".tree-folder-content:eq(0)")
																.append(i)
													} else {
														f.append(i)
													}
		...
		selectItem : function(e) {
			...
			if (this.options.multiSelect) {
				...
				if (g[0] !== d[0]) {
					g.removeClass("tree-selected").find("i").removeClass(
							this.options["selected-icon"]).addClass(
			...
			if (d.hasClass("tree-selected")) {
				...
				d.find("i").removeClass(this.options["selected-icon"])
						.addClass(this.options["unselected-icon"])
			} else {
				d.addClass("tree-selected");
				d.find("i").removeClass(this.options["unselected-icon"])
						.addClass(this.options["selected-icon"]);
				if (this.options.multiSelect) {
					f.push(d.data())
				}
			}
		...
		selectFolder : function(e) {
			...
			if (d.find("." + this.options["close-icon"]).length) {
				...
				f.find("." + this.options["close-icon"] + ":eq(0)")
						.removeClass(this.options["close-icon"]).addClass(
				...
				f.find("." + this.options["open-icon"] + ":eq(0)").removeClass(
						this.options["open-icon"]).addClass(
